...
impl OutputLang {
        ...
        match self {
            OutputLang::C => {
                match t {
                    ItemType::Str(_) => "char*",
                    ItemType::I32(_) => "int",
                    ItemType::I64(_) => "long int",
                    ItemType::U32(_) => "unsigned int",
                    ItemType::U64(_) => "long unsigned int",
                }
            },
            OutputLang::Python => {
                match t {
    ...
    pub fn get_fo_hash_data(&self, name: &str) -> Option<FOHashData> {
        return FO_HASHES.functions.get(name).and_then(|map| map.get(&self.to_string())).cloned();
    }

    pub fn get_so_hash_data(&self, name: &str) -> Option<SOHashData> {
        return SO_HASHES.functions.get(name).and_then(|map| map.get(&self.to_string())).cloned();
    }
